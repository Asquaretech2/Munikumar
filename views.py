    ...
    for movie_data in data['results']:
        ...
        if not Movie.objects.filter(movie_title=movie_data['title']).exists():
            ...
            movie = Movie(
                ...
                video=','.join([video['key'] for video in videos]),
                cast=','.join(cast),
                crew=','.join(director + music_director),
                director=','.join(director),
                music_director=','.join(music_director),
                slug = ''.join(movie_data['title'].split(' '))
...
def publicfeed(request):
    ...
    Publicfeed = sorted(list(mydata) + list(movies) + list(mydata1) + list(mydata3),
...
def Feedpage(request):
    ...
    combined_data = sorted(list(mydata) +list(movies) +list(mydata1) +list(mydata3) , 
    key=lambda x: x.timestamp_field, reverse=True
    )
...
def Poster(request):
    if request.method =='POST':
        form = PosterMoviePhotosForm(request.POST, request.FILES)
        if form .is_valid():
            form.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if CreateLabel.objects.filter(Movie_name=Movie_name).exists() and Movie_name in movie_list:
                ...
                PostText1.objects.filter(Movie_name = Movie_name).update(slug = ''.join(Movie_name.split(' ')))
...
def promo(request, Url_name):
    ...
    all_data = list(chain(mydata, mydata2, mydata1))
    all_data.sort(key=attrgetter('timestamp_field'), reverse=True)
